...
include "map_builder.lua" --其实包含了其他五个lua文件
include "trajectory_builder.lua"

options = {
  map_builder = MAP_BUILDER,                -- map_builder.lua的配置信息   自己设置参数   
  ...
  submap_publish_period_sec = 0.3,          -- 发布数据的时间间隔 发布submap频率
  pose_publish_period_sec = 5e-3,            --发布pose
  trajectory_publish_period_sec = 30e-3,     --发布轨迹

  rangefinder_sampling_ratio = 1.,          -- 传感器数据的采样频率  为0.5的话，来两桢用一祯数据 优点降低计算量 控制不好数据的比例
...
TRAJECTORY_BUILDER_2D.use_online_correlative_scan_matching = false --体素滤波
...
POSE_GRAPH.optimize_every_n_nodes = 160.  --当字图插入160个节点时，后段执行一次优化 一般设置两倍submaps.num_range_data1
